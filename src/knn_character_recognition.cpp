...
KNN_Character_Recognition::KNN_Character_Recognition()
{
}

void KNN_Character_Recognition::readDataSet()
{
...
void KNN_Character_Recognition::train()
{
    kNearest = cv::ml::KNearest::create();

    kNearest->setIsClassifier(true);
    kNearest->setAlgorithmType(cv::ml::KNearest::Types::BRUTE_FORCE);
    kNearest->setDefaultK(10);     //default K is 10 in opencv

    std::cout << "KNN's K: " << kNearest->getDefaultK() << std::endl;

    kNearest->train(trainingData);
}

void KNN_Character_Recognition::classify(cv::Mat matTestingNumbers,std::string &strFinalString)
{
